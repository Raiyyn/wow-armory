...
class phpArmory {
    ...
    * setLanguage
    * 
    * Set the language in which items shall be fetched.
    *
    ...
    * getLanguage
    * 
    * Returns the language in which items are fetched.
    *
    * @return string    $language       language used
    */ 
    ...
    */
    function itemNameFetch($item, $filter = NULL) {
        ...
        if ($this->language != 'en') {
            ...
            $item = urlencode(utf8_encode($item));
        }
        ...
        foreach ($items as $x_item) {
            if ($this->language != 'en') {
                ...
                $item = urldecode(utf8_decode($item));
            }
    ...
    * 
    * @param string[]   $itemRef        The item icon ref from the character or item info array.  
    * @param string[]   example         $itemRef = $char['characterinfo']['charactertab']['items']['item']['0']['icon'];
    * @param string[]   example         $itemRef = $item['itemtooltips']['itemtooltip']['icon'];
    * @return string                    The URL of the icon 
    ...
    */
    function xmlFetch($url, $userAgent = NULL, $timeout = NULL){
        ...
        if (function_exists('curl_init')){
            ...
            curl_setopt ($ch, CURLOPT_HTTPHEADER, array('Accept-language: '.$this->language));
